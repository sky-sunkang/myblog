  ...
  <div>
    <div class="login-box">
      ...
      <form method="post" action="">
        <div class="name">
          <label>账号：</label>
          <input type="text" v-model="username"  tabindex="1" autocomplete="off"/>
        </div>
        <div class="password">
          <label>密 码：</label>
          <input type="password" v-model="password" maxlength="16"  tabindex="2"/>
        </div>
        <div class="code">
          <label>验证码：</label>
          <input type="text" name="" maxlength="4" id="code" tabindex="3"/>
          <div class="codeImg">
            <img src="../../../static/images/captcha.jpeg.jpg"/>
          </div>
        </div>
        <div class="login">
          <button v-on:click="login" type="button" tabindex="5">登录</button>
        </div>
      </form>
    </div>

    <div class="bottom">Copyright © 2018.粤ICP备18040421号</div>
    <div class="screenbg">
      <ul>
      ...
      this.$axios.post('/server/login', {
        ...
        .then(function (response) {
          ...
          if (response.data.code === 200) {
            this.state.token = response.data.resultData
            this.$router.push('index')
            this.$Message.error('登陆成功啦！')
          } else {
            this.$Message.error(response.data.codeDec)
          }
        })
